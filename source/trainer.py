...
def get_captions_from_file(caption_file, dl, tokenizer, model, args):
    # This is where we retrieve the captions from file and embed the captions.
    img_path_to_caption_dict = {}
    with open(caption_file) as f:
        for line in f:
            if '<eol>' in line:
                ...
                assert not img_path in img_path_to_caption_dict
                img_path_to_caption_dict[img_path] = caption
                
    dl_paths = []
    for i in range(len(dl.dataset.imgs)):
        imagename = dl.dataset.imgs[i][0]
        assert imagename[:len(args.data_dir)] == args.data_dir
        dl_paths.append(imagename[len(args.data_dir):])
#     dl_paths = [dl.dataset.imgs[i][0] for i in range(len(dl.dataset.imgs))]
    caption_texts = [img_path_to_caption_dict[dl_paths_i] for dl_paths_i in dl_paths]
    ...
    assert len(text_features) == len(dl.dataset.imgs)
